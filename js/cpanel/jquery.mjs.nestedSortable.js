...
(function( factory ) {
	...
	$.widget("mjs.nestedSortable", $.extend({}, $.ui.sortable.prototype, {
					...
					if (o.isTree && $(itemElement).hasClass(o.collapsedClass) && o.expandOnHover) {
						if (!this.hovering) {
							...
							this.hovering = window.setTimeout(function() {
								...
								$(itemElement).find('> .relative .disclose > i')
		...
		toMenuArray: function(options) {

			var o = $.extend({
					hierarchyLength : 3
				}, this.options, options),
				returnData = [], hierarchy = [], 
				depthLevelSortOrderArray = [], depth = 0;
			
			// depthLevelSortOrderArray index goes from 0 -> hierarchyLength - 1 but references depth from 1 -> hierarchyLength
			depthLevelSortOrderArray[depth] = 1;
			$(this.element).children(o.items).each(function() {
				var level = _recursiveItems(this);
				hierarchy.push(level);
				_pushFlatLevel(level);
			});
			...
			function _recursiveItems(item) {
				...
				if (id) {
					...
					// Create the hierarchy of the item
					for (var i = 0; i < depth; i++) {
						 hierarchyString += _leftPad(depthLevelSortOrderArray[i], o.hierarchyLength);
					}
					
					currentItem = {
						id: id[2],
						name: $('#menu-item-name-' + id[2]).val(),
						url: $(item).attr('cp-url'),
						url_qs: $(item).attr('cp-qs'),
						visible: $(item).attr('cp-visible'),
						css_class: $(item).attr('cp-class'),
						depth: depth,
					...
					depth--;
					depthLevelSortOrderArray[depth]++;
					return currentItem;
				}
			}
			
			// Adds the level to a flat array without the items children
			function _pushFlatLevel(level) {
				// Remove children for the flat object
				var flatLevel = $.extend({}, level);
				delete flatLevel.children;
				returnData.push(flatLevel);
			}
			
			// Pads out a number to however many digits are required
