...
(function($) {
	...
	jQuery.fn.addTag = function(value,options) {
			...
			this.each(function() { 
				...
				if (value !='') { 
					...
					$('<span class="tag">'+value + '&nbsp;&nbsp;<a href="#" title="Remove Item" onclick="return $(\'#'+id + '\').removeTag(\'' + escape(value) + '\');">x</a></span>').insertBefore('#'+id+'_addTag');
					tagslist.push(value);
	...
	jQuery.fn.tagsInput = function(options) { 
		...
		var settings = jQuery.extend({defaultText:'','hide':true,'delimiter':',',autocomplete:{selectFirst:false}},options);
